  ...
  def inference(self, frame: np.ndarray, gt: GroundTruth, verbose: bool = True):
    ...
    for d in range(n_boxes_per_sample[0]):
        ...
        if self.draw:
          ...
          detections = sv.Detections(xyxy=np.array([self.transform_bbox(pred_box, (640, 480))])) # transform normalized cxcywh to xyxy
    ...
    if verbose:
      logger.succ(f"[{self.img_id:04d}] Total: {time.time() - start_:.4f}s | PoEt: {poet_time:.4f}s | x: {t[0]} | y: {t[1]} | z : {t[2]} | RMSE (t): {t_rmse:.4f}, (R): {R_rmse:.4f}")
